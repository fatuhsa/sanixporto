---
name: ui-ux-pro-max
description: Comprehensive design guide for web, mobile, and desktop applications. Contains 84 styles, 192 color palettes, 74 font pairings, 99 UX guidelines, and 25 chart types across 22 technology stacks.
---
# ui-ux-pro-max
...
If the user prefers not to install Python, skip the CLI searches and rely on the inlined rules below (**Common Rules for Professional UI** and **Pre-Delivery Checklist**).
...
| **Review existing UI** | "Review this page for UX issues"、"检查无障碍" | Step 3 (domain search: ux) + Pre-Delivery Checklist |
| **Fix a UI bug** | "Button hover is broken"、"Layout shifts on load" | Step 3 (domain search: ux) → Common Sticking Points |
...
| Dark mode contrast issues | `--domain color "dark mode"` + `--domain ux "color accessible pairs contrast"` |
| Animations feel unnatural | `--domain ux "spring physics"` + `--domain ux "easing"` + `--domain ux "exit faster than enter"` |
| Form UX is poor | `--domain ux "inline validation"` + `--domain ux "error clarity"` + `--domain ux "focus management"` |
| Navigation feels confusing | `--domain ux "nav hierarchy"` + `--domain ux "bottom nav limit"` + `--domain ux "back behavior"` |
| Layout breaks on small screens | `--domain ux "mobile first"` + `--domain ux "breakpoint consistency"` |
| Performance / jank | `--domain ux "virtualize lists"` + `--domain ux "main thread budget"` + `--domain ux "debounce throttle"` |

### Pre-Delivery Checklist

- Run `--domain ux "animation accessibility z-index loading"` as a UX validation pass before implementation
- Re-check High-severity items: `--domain ux "high severity"` and review the top issues for the target platform
...
- 默认图标库使用 **Phosphor (`@phosphor-icons/react`)**。`data/icons.csv`（本 skill 的 data 目录）中列出的只是常用推荐图标，不是完整集合。
- 当推荐表中找不到合适的图标时：
